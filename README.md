...
| --------- | ----------------- | ------------------------------------------------------- | -------- | ---------------------- |
| MY MOON   | [NASA SVS][svs]   | The Moon in my sky — hidden when it's below the horizon | Hourly   | ±30 min (nearest hour) |
| MOON      | [NASA SVS][svs]   | The Moon from Earth's centre — no Earth in frame        | Hourly   | ±30 min (nearest hour) |
...
to wait out — every hour of the year is already available. "Age" above isn't a lag, then; it's how
far the shown frame's hour sits from now, since the card picks whichever hour is closest rather than
always the one just past. The product still ships a year at a time under an id that changes each
December, so both tiles go blank on 1 January until a release adds the new one.
