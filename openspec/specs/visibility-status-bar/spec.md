...
- `<Mode>` is the current sky mode: one of `Day`, `Civil Twilight`, `Nautical Twilight`, `Astronomical Twilight`, `Night`
...
The `.status-bar` element SHALL use `display: flex; justify-content: space-between; align-items: center` so that a left-aligned context group and a right-aligned next-transition group occupy opposite ends of the bar. Each group SHALL be wrapped in a `<span>`. Long text in either span SHALL be clipped with an ellipsis rather than wrapping to a second line.
